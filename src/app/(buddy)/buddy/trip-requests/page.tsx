...
import type { DateFilter, PeopleFilter, SortMode, TripRequestViewModel } from "./components/types";
...
function estimateMatchScore(durationHours: number, groupSize: number, languageCount: number) {
	...
	return Math.min(99, 45 + Math.floor((durationScore + groupScore + languageScore) / 3));
}
...
export default function BuddyTripRequestsPage() {
	...
	const { openTripsQuery } = useTrip({ page: 1, pageSize: 12 });
	...
	const languageOptions = useMemo(
		() =>
			Array.from(
				new Set(
					requests.flatMap((request) => request.preferredLanguages).filter(Boolean),
				),
			),
		[requests],
	);
	...
	const viewData = useMemo(() => {
		const filtered = requests
			.filter((r) => {
				...
				if (!matchesPeopleFilter(r.adults + r.children, peopleFilter)) return false;
				if (
					languageFilter !== "All" &&
	...
	const handleApplyContact = (requestId: string) => {
		...
		toast.success("You have contacted this traveler. Continue the conversation in Messages.");
	};
	...
	return (
		<main className="min-h-[calc(100vh-84px)] bg-[linear-gradient(180deg,#f4f7f6_0%,#f7f7f8_55%,#f8f6f2_100%)]">
			<div className="mx-auto flex w-full max-w-7xl flex-col gap-5 px-4 py-6 md:px-6 md:py-8">
				...
				{/* Grid */}
				<TripRequestsGrid requests={viewData} onApplyContact={handleApplyContact} />
			</div>
		</main>
	);
}