...
import { useAdminOverviewBuddyGrowthTrend } from "@/features/admin/hooks/useAdmin";

function BuddyGrowthTooltip({
  active,
  payload,
  label,
}: {
...
export function OverviewBuddyGrowthTrendCard() {
  const query = useAdminOverviewBuddyGrowthTrend();
  ...
  if (query.isError || !query.data?.data) {
    return (
      <OverviewErrorCard
        title="Unable to load buddy growth trend"
        description="The buddy growth trend endpoint did not return usable chart data."
      />
    );
  }
  ...
  return (
    <div className="booking-muted-panel h-full p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="type-h3 text-foreground">Buddy growth trend</p>
          <p className="mt-1 text-sm text-muted-foreground">
            Weekly onboarding, subscriptions, and completed bookings.
          </p>
        </div>
        <Link
          href="/admin/buddies"
        ...
        {rows.length ? (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={rows} barGap={10}>
              ...
              <YAxis
                ...
                allowDecimals={false}
              />
              <Tooltip cursor={false} content={<BuddyGrowthTooltip />} />
              <Bar
                dataKey="newBuddies"
                name="New buddies"
                fill="#16a34a"
          ...
          <Card className="flex h-full items-center justify-center border-dashed bg-background/70 p-6 text-center shadow-none">
            <CardContent className="p-0">
              <EmptyState
                title="No buddy growth data"
                description="The buddy growth trend endpoint returned no weekly chart rows."
              />
            </CardContent>
          </Card>
        )}
